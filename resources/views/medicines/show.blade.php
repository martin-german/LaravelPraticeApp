...
<div class="container mx-auto px-4 py-8 text-white font-jakarta">
    <h1 class="text-2xl font-bold mb-6">{{ $medicine->name }} részletek</h1>
    
    <div class="bg-[#1C2A29] p-6 rounded shadow-md">
        <h1 class="text-xl font-semibold mb-4">{{ $medicine->name }}</h1>
        <h2 class="text-xs mb-4"> Kategória: {{ $medicine->category->name }}</h2>
        <h3 class="text-base mb-4"> {{$medicine->description}}</h3>
        <h4 class="text-sm mb-4"> Ár: {{$medicine->price}} Ft</h4>
        ...
        <p class="text-sm mb-4"> További információk: 
            <a href="{{ $medicine->link }}" target="_blank" class="text-blue-400 hover:underline">
                {{ Str::limit(trim($medicine->link),45) }}
            </a>
        </p>
        <div class="mt-4">
